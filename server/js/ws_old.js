...
var Connection = cls.Class.extend({
  ...
  close: function (logError) {
    log.info(
      "Closing connection to " + this._connection.handshake.headers["cf-connecting-ip"] + ". Error: " + logError,
    );
  ...
  init: function (port, useOnePort, ip) {
    ...
    if (useOnePort === true) {
      ...
      this._httpServer = http
        .createServer(app)
        .listen(port, this.ip || undefined, function serverEverythingListening() {
    ...
    console.log("~~~~ Remove cors when BE server will be compatible on dev");
    ...
    this._ioServer.on("connection", function webSocketListener(socket) {
      ...
      socket.remoteAddress = socket.handshake.headers["cf-connecting-ip"];
...
WS.socketioConnection = Connection.extend({
  init: function (id, connection, server) {
    ...
    this._connection.on("disconnect", function () {
      log.info("Client closed socket " + self._connection.handshake.headers["cf-connecting-ip"]);
